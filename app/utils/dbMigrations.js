...
dbMigrations.migerateDatabase = async () => {
    ...
    if (version < DATABASE_VERSIONS.ONE) {
        ...
        await userModel({ email: ADMIN.EMAIL, password: password, firstName: ADMIN.FIRST_NAME, lastName: ADMIN.LAST_NAME, userType: USER_TYPES.SUPER_ADMIN, status: STATUS.ACTIVE}).save();
