...
const { userModel,franchaiseModel, dbVersionModel } = require('../models')
let dbMigrations = {};
...
dbMigrations.migerateDatabase = async () => {
    ...
    if (version < DATABASE_VERSIONS.ONE) {
        ...
        await franchaiseModel({ email: ADMIN.EMAIL, password: password, firstName: ADMIN.FIRST_NAME, lastName: ADMIN.LAST_NAME, userType: USER_TYPES.SUPER_ADMIN, status: STATUS.ACTIVE}).save();
